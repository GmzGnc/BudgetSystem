        ...
        <div style={{ display: 'grid', gridTemplateRows: paramOpen ? '1fr' : '0fr', transition: 'grid-template-rows 0.25s ease' }}>
          <div className="overflow-hidden">
            <div className="border-t border-gray-100 dark:border-gray-700 p-3 space-y-4">
              ...
              {birimFiyat && (() => {
                const bud       = ensureArray(birimFiyat.monthly_budget);
                const act       = ensureArray(birimFiyat.monthly_actual);
                ...
                return (
                  <div>
                    <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">
                      Birim Fiyat ({birimFiyat.unit_type ?? 'TL/öğün'})
                    </p>
              ...
              {/* ── Öğün Sayısı — 12 ay kolonlu, accordion ──────────────────── */}
              <div>
                ...
                <div>
                  ...
                  <div className="overflow-x-auto rounded border border-gray-200 dark:border-gray-700">
                    <table className="min-w-[1000px] text-xs table-fixed">
                      ...
                      <thead className="bg-gray-50 dark:bg-gray-800">
                        <tr>
                          <th className="px-2 py-1.5 text-left font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide text-[10px]">Grup</th>
                          {MONTH_LABELS.map((m) => (
                            <th key={m} className="px-1 py-1.5 text-right font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide text-[10px]">{m}</th>
                          ))}
                          <th className="px-2 py-1.5 text-right font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide text-[10px]">Yıllık</th>
                      ...
                      <tbody>
                        {gunParams.map((g) => {
                          const bud       = ensureArray(g.monthly_budget);
                          const act       = ensureArray(g.monthly_actual);
                          const budAnnual = bud.reduce((a, b) => a + b, 0);
                          const actAnnual = act.reduce((a, b) => a + b, 0);
                          const hasActual = act.some((v) => v !== 0);
                          return (
                            <React.Fragment key={g.param_code}>
                              <tr className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-800/20 hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                                <td className="px-2 py-1 text-gray-700 dark:text-gray-300 font-medium truncate">{g.label}</td>
                                {bud.map((v, mi) => (
                                  <td key={mi} className="px-1 py-1 text-right font-mono text-gray-600 dark:text-gray-400">{v === 0 ? '—' : v.toLocaleString('tr-TR')}</td>
                                ))}
