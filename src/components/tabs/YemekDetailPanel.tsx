...
// Öğün (meal count) group hierarchy
const OGUN_GROUPS = [
  { id: 'toplam',    label: 'Toplam',    parentCode: 'ogun_toplam',    childPrefix: null          as string | null },
  { id: 'gyg',       label: 'GYG',       parentCode: 'ogun_gyg',       childPrefix: 'ogun_gyg_'   as string | null },
  { id: 'ictas',     label: 'İçtaş',     parentCode: 'ogun_ictas',     childPrefix: null          as string | null },
  { id: 'operasyon', label: 'Operasyon', parentCode: 'ogun_operasyon', childPrefix: 'ogun_ops_'   as string | null },
  { id: 'kamu',      label: 'Kamu',      parentCode: 'ogun_kamu',      childPrefix: 'ogun_kamu_'  as string | null },
];
...
export default function YemekDetailPanel({ dark, lineItems }: Props) {
  ...
  const [openOgunGroups, setOpenOgunGroups] = useState<Set<string>>(new Set());

  // ── derive data from lineItems ──────────────────────────────────────────────

  const totalItem = useMemo(
    () => lineItems.find((i) => i.category_code === 'yemek' && i.row_type === 'total') ?? null,
    [lineItems]
  );
  ...
  const depts = useMemo(
    () => lineItems.filter((i) => i.category_code === 'yemek' && i.row_type === 'dept'),
    [lineItems]
  );

  const deptItems = (deptCode: string) =>
    lineItems.filter((i) => i.category_code === 'yemek' && i.row_type === 'item' && i.dept_code === deptCode);

  const paramByCode = useMemo(() => {
    const m = new Map<string, LineItem>();
    lineItems
      .filter((i) => i.category_code === 'yemek' && i.row_type === 'param')
      .forEach((i) => { if (i.param_code) m.set(i.param_code, i); });
    return m;
  }, [lineItems]);

  const birimFiyat = paramByCode.get('birim_fiyat') ?? null;

  const gunParams = useMemo(
    () => lineItems.filter((i) => i.category_code === 'yemek' && i.row_type === 'param' && i.param_code?.startsWith('gun_')),
    [lineItems]
  );
    ...
    <div className="mt-4 space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Fiili YTD',    value: fmtM(ytdActual),    sub: `Oca–${activeLabel} 2025`,   cls: 'text-amber-600 dark:text-amber-400',                                                          border: 'border-amber-200 dark:border-amber-800' },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 shadow-sm">
          <p className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-3">
            Aylık Bütçe vs Fiili — Yemek 2025
          </p>
          <ChartWrapper height={180}>
            {(w, h) => (
              <ComposedChart width={w} height={h} data={chartMonthly} margin={{ top: 4, right: 8, bottom: 0, left: 8 }}>
                ...
                <Bar dataKey="Fiili" fill="#fb923c" radius={[3, 3, 0, 0]} maxBarSize={18} />
                <Line type="monotone" dataKey="Bütçe" stroke="#f59e0b" strokeWidth={2} dot={{ r: 2.5, fill: '#f59e0b' }} activeDot={{ r: 4 }} />
              </ComposedChart>
            )}
          </ChartWrapper>
        </div>
            ...
            <div key={deptCode} className="border-b border-gray-100 dark:border-gray-800 last:border-0">
              ...
              >
                ...
                <div className="hidden sm:flex items-center gap-4 text-[10px]">
                  ...
                  <span className="text-gray-400 dark:text-gray-500">YTD Fiili: <span className="font-mono font-semibold text-amber-600 dark:text-amber-400">{fmtM(deptActYTD)}</span></span>
              ...
              <div style={{ display: 'grid', gridTemplateRows: isOpen ? '1fr' : '0fr', transition: 'grid-template-rows 0.25s ease' }}>
                <div className="overflow-hidden">
                  <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30 p-3 space-y-3">
                    ...
                    {deptCode === 'kilyos' && (
                      <div className="px-2 py-2 bg-amber-50/60 dark:bg-amber-950/20 rounded border border-amber-200 dark:border-amber-800">
                        <p className="text-[11px] text-amber-700 dark:text-amber-400 italic">
                          Endeks bağlı — tek satır. Yemek toplamının yaklaşık %86&apos;sını oluşturur.
                        </p>
                      </div>
                    )}
                    ...
                    {items.length > 0 && (
                      <div className="rounded border border-gray-200 dark:border-gray-700 overflow-hidden">
                        <div className="px-3 py-2 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                          <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Alt Kalemler — {items.length} adet</p>
                        </div>
                        <div className="overflow-x-auto">
                          <table className="w-full min-w-[400px] text-xs">
                            ...
                            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                              {items.map((item) => {
                                const imb     = ensureArray(item.monthly_budget);
                                const ima     = ensureArray(item.monthly_actual);
                                const iAnnual = imb.reduce((a, b) => a + b, 0);
                                const iBudYTD = imb.slice(0, activeMonth + 1).reduce((a, b) => a + b, 0);
                                const iActYTD = ima.slice(0, activeMonth + 1).reduce((a, b) => a + b, 0);
                                const iSapma  = iActYTD - iBudYTD;
                                const iSapPct = iBudYTD > 0 ? (iSapma / iBudYTD) * 100 : 0;
                                return (
                                  <tr key={item.item_code} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                                    ...
                                    <td className="px-3 py-1.5 text-right font-mono text-gray-600 dark:text-gray-400">{fmtM(iAnnual)}</td>
                                    <td className="px-3 py-1.5 text-right font-mono text-amber-600 dark:text-amber-400">{iActYTD > 0 ? fmtM(iActYTD) : '—'}</td>
                                    <td className={`px-3 py-1.5 text-right font-semibold ${iSapma > 0 ? 'text-red-500 dark:text-red-400' : iSapma < 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-400 dark:text-gray-500'}`}>
                                      {iActYTD > 0 ? `${iSapPct >= 0 ? '+' : ''}${iSapPct.toFixed(1)}%` : '—'}
                                    </td>
                                  </tr>
                                );
                              })}
                    ...
                    {items.length === 0 && deptCode !== 'kilyos' && (
                      <p className="text-[11px] text-gray-400 dark:text-gray-500 italic px-1">Bu departman için alt kalem detayı bulunmamaktadır.</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        >
          <p className="text-xs font-semibold text-gray-600 dark:text-gray-300">Parametre Detayı — Birim Fiyat, Öğün & Gün Sayısı</p>
          <span className="text-gray-400 dark:text-gray-500 text-[10px]" style={{ display: 'inline-block', transform: paramOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}>▾</span>
        </button>
        ...
        <div style={{ display: 'grid', gridTemplateRows: paramOpen ? '1fr' : '0fr', transition: 'grid-template-rows 0.25s ease' }}>
          <div className="overflow-hidden">
            <div className="border-t border-gray-100 dark:border-gray-700 p-3 space-y-4">
              ...
              {birimFiyat && (
                <div>
                  <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Birim Fiyat</p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {[
                      { label: 'Bütçe (Aylık)',  value: fmtM(ensureArray(birimFiyat.monthly_budget)[activeMonth] ?? 0) },
                      { label: 'Fiili (Aylık)',   value: fmtM(ensureArray(birimFiyat.monthly_actual)[activeMonth] ?? 0) },
                      { label: 'Birim',           value: birimFiyat.unit_type ?? 'TL/öğün' },
                    ].map(({ label, value }) => (
                      <div key={label} className="bg-gray-50 dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 px-3 py-2">
                        <p className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wide">{label}</p>
                        <p className="text-xs font-bold mt-0.5 font-mono text-gray-800 dark:text-gray-100">{value}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Öğün Sayısı groups */}
              <div>
                <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Günlük Öğün Sayısı</p>
                <div className="rounded border border-gray-200 dark:border-gray-700 overflow-hidden">
                  {OGUN_GROUPS.map((grp) => {
                    ...
                    const isGrpOpen   = openOgunGroups.has(grp.id);

                    return (
                      <div key={grp.id} className="border-b border-gray-100 dark:border-gray-800 last:border-0">
                        <div
                          role={hasChildren ? 'button' : undefined}
                          onClick={hasChildren ? () => setOpenOgunGroups((prev) => {
                            const next = new Set(prev);
                            if (next.has(grp.id)) next.delete(grp.id); else next.add(grp.id);
                            return next;
                          }) : undefined}
                          className={`flex items-center gap-2 px-3 py-2 text-xs ${hasChildren ? 'cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/60' : ''} ${grp.id === 'toplam' ? 'bg-gray-50 dark:bg-gray-800/40 font-semibold' : ''} transition-colors`}
                        >
                          {hasChildren ? (
                            <span className="text-gray-400 dark:text-gray-500 text-[9px] flex-shrink-0" style={{ display: 'inline-block', transform: isGrpOpen ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}>▶</span>
                          ) : (
                            <span className="w-[9px] flex-shrink-0" />
                          )}
                          <span className="flex-1 text-gray-700 dark:text-gray-300">{grp.label}</span>
                          <span className="w-12 text-right font-mono text-gray-600 dark:text-gray-400">{fmtNum(budgetVal)}</span>
                          <span className="w-12 text-right font-mono text-amber-600 dark:text-amber-400">{fmtNum(actualVal)}</span>
                              ...
                              <table className="w-full text-xs border-t border-gray-100 dark:border-gray-800">
                                <thead className="bg-gray-50/80 dark:bg-gray-800/50">
                                  <tr>
                                    ...
                                    <th className="px-3 py-1 text-right font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide w-14">Büt.</th>
                                    <th className="px-3 py-1 text-right font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide w-14">Fili</th>
                                  ...
                                  {childItems.map((ci) => {
                                    ...
                                    return (
                                      <tr key={ci.param_code} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                                        ...
                                        <td className="px-3 py-1 text-right font-mono text-gray-500 dark:text-gray-500">{fmtNum(ciBud)}</td>
                                        <td className="px-3 py-1 text-right font-mono text-amber-500 dark:text-amber-500">{fmtNum(ciAct)}</td>
              ...
              {gunParams.length > 0 && (
                <div>
                  <p className="text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Gün Sayısı Ortalaması</p>
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead className="bg-gray-50 dark:bg-gray-800">
                        <tr>
                          {['Grup', 'Bütçe (Aylık)', 'Fiili (Aylık)', 'Fark'].map((h, i) => (
                      ...
                      <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                        {gunParams.map((g) => {
                          const gb   = ensureArray(g.monthly_budget);
                          const ga   = ensureArray(g.monthly_actual);
                          const bud  = gb[activeMonth] ?? 0;
                          const act  = ga[activeMonth] ?? 0;
                          const diff = act - bud;
                          return (
                            <tr key={g.param_code} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                              <td className="px-3 py-1.5 text-gray-700 dark:text-gray-300">{g.label}</td>
                              <td className="px-3 py-1.5 text-right font-mono text-gray-600 dark:text-gray-400">{bud > 0 ? bud : '—'}</td>
                              <td className="px-3 py-1.5 text-right font-mono text-amber-600 dark:text-amber-400">{act > 0 ? act : '—'}</td>
