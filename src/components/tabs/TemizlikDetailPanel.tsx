...
const DEPT_COLORS  = ['#6366f1', '#f59e0b', '#10b981', '#3b82f6', '#8b5cf6', '#ec4899'];
...
const HC_GROUPS = [
  ...
  { id: 'kamu',      label: 'Kamu',      parentCode: 'kisi_kamu',      childPrefix: 'kisi_kamu_'  as string | null },
  ...
  { id: 'kilyos',    label: 'Kilyos',    parentCode: null,             childPrefix: null,
    note: 'Kişi bazlı hesap yok — endeks bağlı toplam tutar' },
];
...
export default function TemizlikDetailPanel({ dark, lineItems }: Props) {
  ...
  const totalItem = useMemo(
    () => lineItems.find((i) => i.category_code === 'temizlik' && i.row_type === 'total') ?? null,
    [lineItems]
  );
  ...
  const depts = useMemo(
    () => lineItems.filter((i) => i.category_code === 'temizlik' && i.row_type === 'dept'),
    [lineItems]
  );

  const deptItems = (deptCode: string) =>
    lineItems.filter((i) => i.category_code === 'temizlik' && i.row_type === 'item' && i.dept_code === deptCode);

  const unitWages = useMemo(
    () => lineItems.filter((i) => i.category_code === 'temizlik' && i.row_type === 'param' && i.param_code?.startsWith('ucret_')),
    [lineItems]
  );
  ...
  const paramByCode = useMemo(() => {
    ...
    lineItems
      .filter((i) => i.category_code === 'temizlik' && i.row_type === 'param')
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 shadow-sm">
          <p className="text-xs font-semibold text-gray-600 dark:text-gray-300 mb-3">
            Aylık Bütçe vs Fiili — Temizlik 2025
          </p>
          <ChartWrapper height={180}>
            {(w, h) => (
              <ComposedChart width={w} height={h} data={chartMonthly} margin={{ top: 4, right: 8, bottom: 0, left: 8 }}>
                ...
                <Line type="monotone" dataKey="Bütçe" stroke="#10b981" strokeWidth={2} dot={{ r: 2.5, fill: '#10b981' }} activeDot={{ r: 4 }} />
              </ComposedChart>
            )}
          </ChartWrapper>
        </div>
          ...
          <div className="flex items-center gap-2">
            <PieChart width={140} height={140}>
              <Pie
                data={depts.map((d) => ({
                  name: d.label,
                  value: ensureArray(d.monthly_budget).reduce((a, b) => a + b, 0),
                }))}
                dataKey="value"
                nameKey="name"
                cx="50%" cy="50%"
            ...
            <div className="flex-1 space-y-1">
              {depts.map((d, i) => {
                const deptAnnual = ensureArray(d.monthly_budget).reduce((a, b) => a + b, 0);
